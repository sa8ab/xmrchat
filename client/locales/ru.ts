export default {
  ...
  quickAndEasy: {
    ...
    description:
      "Зрителям не нужно создавать учётную запись, подтверждать email или вводить данные кредитной карты. Фанаты заходят на вашу страницу чаевых, вводят своё имя, сообщение, сумму, сканируют QR-код для оплаты и отправляют чаевые в XMR.",
  },
  openSource: {
    title: "Открытый код",
    description:
  ...
  accepted: {
    title: "Принимается",
    description:
      "Избегайте хлопот с продажей криптовалюты на централизованной бирже. Покупайте товары и услуги напрямую у продавцов, принимающих Monero, на {xmrbazaar} и {monerica} или через консьерж-сервис {shopinbit}.",
  },
  andMore: {
  ...
  forContentCreators: {
    ...
    description: {
      ...
      streamyard: `Перейдите в пункт меню "Edit tip page". Убедитесь, что вы добавили имя пользователя своего канала на Twitch. При запуске трансляции в Streamyard добавьте Twitch в качестве назначения. Бот XMRChat (xmr-chat на Twitch) будет отправлять сообщение о чаевых на Twitch, и вы сможете отображать его на экране через Streamyard. Проверьте настройки Automod на Twitch, так как они могут отфильтровывать нужные суперчаты. Отображаемый логотип бота xmrchat лучше всего смотрится с настройкой "Minimal".`,
